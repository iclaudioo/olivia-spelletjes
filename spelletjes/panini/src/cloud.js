...
(() => {
  ...
  function badge(message, kind = 'idle') {
    const show = () => {
      ...
      if (!el) {
        ...
        el.style.cssText = 'position:fixed;right:10px;top:72px;z-index:80;padding:8px 10px;border-radius:999px;font:800 11px system-ui;background:rgba(8,17,29,.78);color:#fff;border:1px solid rgba(255,255,255,.18);backdrop-filter:blur(12px);box-shadow:0 8px 30px rgba(0,0,0,.25)';
        document.body.appendChild(el);
      }
      const dot = kind === 'ok' ? '🟢' : kind === 'error' ? '🔴' : kind === 'sync' ? '🟡' : '☁️';
  ...
  function normalise(state) {
    ...
    for (const [code, amount] of Object.entries(s.trades || {})) {
      ...
      if (code && n > 0) trades[code] = Math.floor(n);
    }
    return {
      ...s,
      cloudSchema: 1,
      teams,
      trades,
      newOnes: uniqueStrings(s.newOnes),
  ...
  function mergeStates(a, b) {
    ...
    for (const [code, stickers] of Object.entries(right.teams)) teams[code] = uniqueNums([...(teams[code] || []), ...stickers]);

    const trades = { ...left.trades };
    for (const [code, amount] of Object.entries(right.trades)) trades[code] = Math.max(Number(trades[code] || 0), Number(amount || 0));
    ...
    return {
      ...
      teams,
      trades,
      newOnes: uniqueStrings([...(left.newOnes || []), ...(right.newOnes || [])]).slice(-80),
  ...
  async function fetchCloud() {
    const response = await fetch(`${ENDPOINT}?id=eq.${encodeURIComponent(CLOUD_ROW_ID)}&select=state,updated_at`, {
      headers: {
        apikey: SUPABASE_KEY,
  ...
  async function pushCloud(state) {
    ...
    const response = await fetch(`${ENDPOINT}?id=eq.${encodeURIComponent(CLOUD_ROW_ID)}`, {
      method: 'PATCH',
      headers: {
        apikey: SUPABASE_KEY,
        Authorization: `Bearer ${SUPABASE_KEY}`,
  ...
  function schedulePush(delay = 650) {
    ...
    pushTimer = setTimeout(async () => {
      if (pushing) return schedulePush(1200);
      pushing = true;
      badge('opslaan...', 'sync');
      try {
  ...
  async function boot() {
    ...
    const before = normalise(parseState(beforeRaw));
    try {
      const cloud = await fetchCloud();
      const merged = mergeStates(cloud, before);
      const mergedRaw = JSON.stringify(merged);
      if (mergedRaw !== beforeRaw) originalSetItem(STORE, mergedRaw);
      bootDone = true;
      await pushCloud(merged);
      badge('cloud actief', 'ok');
      if (mergedRaw !== beforeRaw && !sessionStorage.getItem('olivia-panini-cloud-reloaded')) {
